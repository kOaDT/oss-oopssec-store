...
The flag for this vulnerability is: **OSS{m4ss_4ss1gnm3nt_vuln3r4b1l1ty}**

The flag can be retrieved by:

1. Navigating to the Sign Up page
2. Intercepting the signup request (using browser dev tools or a proxy)
3. Adding `"role": "ADMIN"` to the request payload
4. Submitting the modified request to create an admin account
5. Accessing the admin panel where the flag will be displayed (since the account is an admin but not one of the expected admin accounts)
